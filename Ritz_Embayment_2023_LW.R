...
LW_YOY<- LW_YOY_Raw |>
  ...
  filter(Species %in% c("ICTALURID", "CARP", "CYPRINIDAE", "KILLI", 
                        "LEPOMIS", "LMB", "SMB")) |>
...
LW_YOY_SUM<- LW_YOY |>
  group_by(Species, Study_Day, Location) |>
...
ggplot(LW_YOY, aes(Date, log(Length), group=Location, color=Location, 
                   lty=Location,fill=Location))+
  geom_point()+
  geom_smooth(method="lm")+ scale_color_manual(values=c("#003366", "#3366CC"))+
  scale_fill_manual(values=c("#003366", "#3366CC"))+
  theme_bw()+
  theme(legend.position = c(.88,.20), legend.text = element_text(size=14), legend.title = element_text(size = 15),
...
ggsave("Figure_4.png", dpi=300, height=8, width=12)
